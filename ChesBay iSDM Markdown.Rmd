...
w <- sapply(tiles, 
            function(p) rgeos::area.poly(rgeos::intersect(as(cbind(p$x, p$y), "gpc.poly"), poly.gpc)))
nv <- mesh$n
...
A.ebird <- inla.spde.make.A(mesh, 
                            loc = rbind(mesh$loc[,1:2], mesh$loc[,1:2], mesh$loc[,1:2], 
                                        mesh$loc[,1:2], as.matrix(ebird.xy[, c("X", "Y")])),
                            group = c(rep(1:4, each = nrow(mesh$loc)), ebird.temp$Season), 
                            group.mesh = mesh.t)
...
### PRESENCE-ONLY DATA
#Ebird
ebird.stk <- inla.stack(data = list(y = cbind(y.ebird.pp, NA, NA),
                                    e = e.ebird.pp),
                        effects = list(list(cbind(data.frame(int.EBird = rep(1, nv*4 + n.ebird),
                                                   Season = c(rep(1:4, each = nv), ebird.temp$Season),
                                                   Week2 = c(rep(c(1,3,7,10), each = nv), ebird.temp$Week2),
                                                   Month = c(rep(c(1,3,7,10)*4, each = nv), ebird.temp$Month)),
                                                  rbind(integrat.covs, integrat.covs, integrat.covs,
                                                        integrat.covs, ebird.env))),
                                       EBird_field = 1:spde$n.spde,
                                       iset),
                        A=list(1, A.ebird.pp, A.ebird),
                        tag="ebird_data")

### PRESENCE-ABSENCE DATA
bbs.stk <- inla.stack(data = list(y = cbind(NA, bbs.data$Occ, NA),
                                Ntrials = rep(1, nrow(bbs.data))),
...
cbc.stk <- inla.stack(data = list(y = cbind(NA, NA, cbc.data$Occ),
                                Ntrials = rep(1, nrow(cbc.data))),
...
all.stk <- inla.stack(cbc.stk, bbs.stk, ebird.stk)

```

Once everything is packaged, we can setup the model. 

```{r}
#Formula
isdm.formula = y ~  -1 + int.EBird + int.BBS + int.CBC +
  f(i, model = spde, group = i.group, control.group = list(model = 'ar1', hyper = h.spec)) +
  f(Month, model = 'ar1', hyper = prior.week2, cyclic = T) + 
  f(EBird_field, model = spde) +
  f(BBS_field, copy = 'EBird_field', fixed = TRUE) +
  f(CBC_field, copy = 'EBird_field', fixed = TRUE)
...
result <- inla(isdm.formula, 
               family = c("poisson", "binomial", "binomial"),
                 ...
                 Ntrials = inla.stack.data(all.stk)$Ntrials,
...
##project the mesh onto the initial simulated grid 100x100 cells in dimension
  ...
  xmean1 <- inla.mesh.project(proj1, 
                              result$summary.random$EBird_field$mean)
  
  
  image.plot(min_x:max_x,min_y:max_y,
             xmean1, 
             col=tim.colors(),
             xlab='', ylab='',
