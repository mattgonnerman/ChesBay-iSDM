...
code <- nimbleCode({
  ...
  for(s in 1:ntotspecies){
    for(i in 1:n.cells){
      ...
      log(lambda[i,s]) <- beta0.lambda[s] + inprod(beta.lambda[s, 1:5], grid.covs[i, 1:5])# + gamma.lambda[i,s]
      p.nb[i,s] <- r.nb[s]/(r.nb[s] + lambda[i,s])
  ...
  for(s in 1:bbl.nspecies){
    for(j in 1:bbl.nsurveys){
      ...
      K.bbl[j,s] <- z[j,bbl.sp[s]]*N[j,bbl.sp[s]] + 0.000001
      y.bbl[j,s] ~ dbinom(K.bbl[j,s], p.bbl[j,s])
    }
  }
  ...
  for(s in 1:ntotspecies){beta0.psi[s] ~ dnorm(0, sd = 100)}
  ...
  for(s in 1:ntotspecies){beta0.lambda[s] ~ dnorm(0, sd = 100)}
  ...
  # for(s in 1:ntotspecies){
  #   spacesigma.psi[s] ~ dunif(0,5)
  #   spacetau.psi[s] <- 1/(spacesigma.psi[s]*spacesigma.psi[s])
  #   gamma.psi[1:n.cells, s] ~ dcar_normal(adj[1:nadj],weights[1:nadj],num[1:n.cells],spacetau.psi[s])
  # 
